...
using AnnaAnimalChatWebSite.Infrastructure.Enums;

namespace AnnaAnimalChatWebSite.Infrastructure.Extensions
{
    public static class CheckBoxListExtensions
    {
            ...
            IDictionary<string, object> htmlAttributes)
        {
            ...
            builder.MergeAttribute("value", info.Value);
            builder.MergeAttribute("name", name);
            builder.MergeAttribute("id", $"{name}_{info.Value}");
            sb.Append(builder);

            var labelBuilder = new TagBuilder("label");
            labelBuilder.MergeAttribute("for", $"{name}_{info.Value}");
            labelBuilder.InnerHtml =info.Text;
            sb.Append(labelBuilder);
